...
namespace JBooth.BetterLit
{
    ...
    //[KeywordEnum(None, Alpha, Cutout)] _WireAlpha("Alpha Mode", Float) = 0
        ...
        public void DoVertexWind(MaterialEditor materialEditor, MaterialProperty[] props)
        {
            if (!mat.HasProperty("_ShiverDrag"))
                return;
            if (DrawRollupKeywordToggle(mat, "Vertex Wind", "_VERTEXWIND"))
            {
                materialEditor.FloatProperty(FindProperty("_InitialBend", props), "Initial Bend");
                materialEditor.FloatProperty(FindProperty("_Stiffness", props), "Stiffness");
                materialEditor.FloatProperty(FindProperty("_Drag", props), "Drag");
                materialEditor.FloatProperty(FindProperty("_ShiverDrag", props), "Shiver Drag");
            }
        }

        enum TerrainTintMode
        {
            Tint,
            HeightTint
        }
        public void DoTerrainTintDetail(MaterialEditor materialEditor, MaterialProperty[] props)
        {
            if (!mat.HasProperty("_TerrainBlendHeight"))
                return;
            if (DrawRollupKeywordToggle(mat, "Terrain Tint", "_TERRAINBLENDALBEDO_TINT"))
            {
                TerrainTintMode mode = TerrainTintMode.Tint;
                if (mat.IsKeywordEnabled("_TERRAINBLENDALBEDO_HEIGHTTINT"))
                {
                    mode = TerrainTintMode.HeightTint;
                }
                EditorGUILayout.HelpBox("Requires the Folliage Renderer with heights and albedo provided to the material for use", MessageType.Info);
                EditorGUI.BeginChangeCheck();
                mode = (TerrainTintMode)EditorGUILayout.EnumPopup("Mode", mode);
                if (EditorGUI.EndChangeCheck())
                {
                    mat.DisableKeyword("_TERRAINBLENDALBEDO_HEIGHTTINT");
                    if (mode == TerrainTintMode.HeightTint)
                    {
                        mat.EnableKeyword("_TERRAINBLENDALBEDO_HEIGHTTINT");
                    }
                }
                if (mode == TerrainTintMode.HeightTint)
                {
                    materialEditor.FloatProperty(FindProperty("_TerrainBlendHeight", props), "Blend Height");
                }
            }

        }

        public void DoTerrainAlignedDetail(MaterialEditor materialEditor, MaterialProperty[] props)
        {
            if (!mat.HasProperty("_TerrainHeightC"))
                return;
            if (DrawRollupKeywordToggle(mat, "Terrain Aligned Detail", "_TERRAINALIGNEDDETAIL"))
            {
                EditorGUILayout.HelpBox("Requires the Folliage Renderer with heights provided to the material for use", MessageType.Info);
                materialEditor.FloatProperty(FindProperty("_TerrainYOffset", props), "Y Offset");
            }
        }
        ...
        public void DoBakery(MaterialEditor materialEditor, MaterialProperty[] props)
        {
            ...
            {
                LightMode lightMode = LightMode.Standard;
                if (mat.IsKeywordEnabled("_LIGHTMAPMODE_RNM"))
                {
                    lightMode = LightMode.RNM;
                }
                else if (mat.IsKeywordEnabled("_LIGHTMAPMODE_SH"))
                {
                    lightMode = LightMode.SH;
                }
                else if (mat.IsKeywordEnabled("_LIGHTMAPMODE_MONOSH"))
                {
                    lightMode = LightMode.MonoSH;
                }
                else if (mat.IsKeywordEnabled("_LIGHTMAPMODE_VERTEX"))
                {
                    lightMode = LightMode.Vertex;
                }
                else if (mat.IsKeywordEnabled("_LIGHTMAPMODE_VERTEXDIRECTIONAL"))
                {
                    lightMode = LightMode.VertexDirectional;
                }
                else if (mat.IsKeywordEnabled("_LIGHTMAPMODE_VERTEXSH"))
                {
                    lightMode = LightMode.VertexSH;
                }
                else if (mat.IsKeywordEnabled("_LIGHTMAPMODE_VERTEXMONOSH"))
                {
                    lightMode = LightMode.VertexMonoSH;
                }
                else if (mat.IsKeywordEnabled("_LIGHTMAPMODE_VOLUME"))
                {
                    lightMode = LightMode.VertexVolume;
                }
                ...
                if (newLightMode != lightMode)
                {
                    ...
                    mat.DisableKeyword("_LIGHTMAPMODE_MONOSH");
                    ...
                    mat.DisableKeyword("_LIGHTMAPMODE_VERTEXMONOSH");
                    mat.DisableKeyword("_LIGHTMAPMODE_VOLUME");

                    if (newLightMode == LightMode.RNM)
                    {
                        mat.EnableKeyword("_LIGHTMAPMODE_RNM");
                    }
                    else if (newLightMode == LightMode.SH)
                    {
                        mat.EnableKeyword("_LIGHTMAPMODE_SH");
                    }
                    else if (newLightMode == LightMode.MonoSH)
                    {
                        mat.EnableKeyword("_LIGHTMAPMODE_MONOSH");
                    }
                    else if (newLightMode == LightMode.Vertex)
                    {
                        mat.EnableKeyword("_LIGHTMAPMODE_VERTEX");
                    }
                    else if (newLightMode == LightMode.VertexDirectional)
                    {
                        mat.EnableKeyword("_LIGHTMAPMODE_VERTEXDIRECTIONAL");
                    }
                    else if (newLightMode == LightMode.VertexSH)
                    {
                        mat.EnableKeyword("_LIGHTMAPMODE_VERTEXSH");
                    }
                    else if (newLightMode == LightMode.VertexMonoSH)
                    {
                        mat.EnableKeyword("_LIGHTMAPMODE_VERTEXMONOSH");
                    }
                    else if (newLightMode == LightMode.VertexVolume)
                    {
                        mat.EnableKeyword("_LIGHTMAPMODE_VOLUME");
                    }
                }

                

                ShadowMaskMode shadowMaskMode = ShadowMaskMode.Default;
                if (mat.IsKeywordEnabled("_BAKERY_SHADOWMASKMODE_VERTEX"))
                {
                    shadowMaskMode = ShadowMaskMode.Vertex;
                }
                else if (mat.IsKeywordEnabled("_BAKERY_SHADOWMASKMODE_VOLUME"))
                {
                    shadowMaskMode = ShadowMaskMode.Volume;
                }

                var newShadowMaskMode = (ShadowMaskMode)EditorGUILayout.EnumPopup("Shadow Mask Mode", shadowMaskMode);
                if (newShadowMaskMode != shadowMaskMode)
                {
                    mat.DisableKeyword("_BAKERY_SHADOWMASKMODE_VERTEX");
                    mat.DisableKeyword("_BAKERY_SHADOWMASKMODE_VOLUME");
                    mat.DisableKeyword("_BAKERY_SHADOWMASKMODE_DEFAULT");
                    if (newShadowMaskMode == ShadowMaskMode.Default)
                    {
                        mat.EnableKeyword("_BAKERY_SHADOWMASKMODE_DEFAULT");
                    }
                    else if (newShadowMaskMode == ShadowMaskMode.Vertex)
                    {
                        mat.EnableKeyword("_BAKERY_SHADOWMASKMODE_VERTEX");
                    }
                    else if (newShadowMaskMode == ShadowMaskMode.Volume)
                    {
                        mat.EnableKeyword("_BAKERY_SHADOWMASKMODE_VOLUME");
                    }
                }

                LMSpecMode lmSpecMode = LMSpecMode.None;
                if (mat.IsKeywordEnabled("_BAKERY_LMSPECMODE_REPLACEREFLECTION"))
                {
                    lmSpecMode = LMSpecMode.ReplaceReflection;
                }
                else if (mat.IsKeywordEnabled("_BAKERY_LMSPECMODE_ADD"))
                {
                    lmSpecMode = LMSpecMode.Add;
                }
                else if (mat.IsKeywordEnabled("_BAKERY_LMSPECMODE_OCCLUDEREFLECTION"))
                {
                    lmSpecMode = LMSpecMode.OccludeReflection;
                }

                var newLmSpecMode = (LMSpecMode)EditorGUILayout.EnumPopup("Specular Mode", lmSpecMode);
                if (newLmSpecMode != lmSpecMode)
                {
                    mat.DisableKeyword("_BAKERY_LMSPECMODE_NONE");
                    mat.DisableKeyword("_BAKERY_LMSPECMODE_REPLACEREFLECTION");
                    mat.DisableKeyword("_BAKERY_LMSPECMODE_ADD");
                    mat.DisableKeyword("_BAKERY_LMSPECMODE_OCCLUDEREFLECTION");
                    if (newLmSpecMode == LMSpecMode.None)
                    {
                        mat.EnableKeyword("_BAKERY_LMSPECMODE_NONE");
                    }
                    else if (newLmSpecMode == LMSpecMode.ReplaceReflection)
                    {
                        mat.EnableKeyword("_BAKERY_LMSPECMODE_REPLACEREFLECTION");
                    }
                    else if (newLmSpecMode == LMSpecMode.Add)
                    {
                        mat.EnableKeyword("_BAKERY_LMSPECMODE_ADD");
                    }
                    else if (newLmSpecMode == LMSpecMode.OccludeReflection)
                    {
                        mat.EnableKeyword("_BAKERY_LMSPECMODE_OCCLUDEREFLECTION");
                    }
                }
                ...
                bool volRotY = mat.IsKeywordEnabled("BAKERY_VOLROTATIONY");
                bool volRot = mat.IsKeywordEnabled("BAKERY_VOLROTATION");
                bool bicubic = mat.IsKeywordEnabled("BAKERY_BICUBIC");
                float roMult = mat.GetFloat("_BAKERY_LMSPECMODE_OCCLUDEREFLECTION_MUL");
                float roAdd = mat.GetFloat("_BAKERY_LMSPECMODE_OCCLUDEREFLECTION_ADD");
                float roSMT = mat.GetFloat("_BAKERY_LMSPECMODE_OCCLUDEREFLECTION_SMT");
                
                
                EditorGUI.BeginChangeCheck();
                
                if (mat.IsKeywordEnabled("_BAKERY_LMSPECMODE_OCCLUDEREFLECTION"))
                {
                    roMult = EditorGUILayout.FloatField("Reflection occlusion multiplier", roMult);
                    roAdd = EditorGUILayout.FloatField("Reflection occlusion offset", roAdd);
                    roSMT = EditorGUILayout.Slider("Reflection occlusion reference smoothness", roSMT, 0, 1);
                }
                shonlinear = EditorGUILayout.Toggle("SH On Linear", shonlinear);
                bicubic = EditorGUILayout.Toggle("BiCubic", bicubic);
                volRotY = EditorGUILayout.Toggle("Volume Rotation Y", volRotY);
                volRot = EditorGUILayout.Toggle("Volume Rotation", volRot);
                if (EditorGUI.EndChangeCheck())
                {
                    mat.DisableKeyword("BAKERY_SHNONLINEAR");
                    mat.DisableKeyword("BAKERY_VOLROTATIONY");
                    mat.DisableKeyword("BAKERY_VOLROTATION");
                    mat.DisableKeyword("BAKERY_BICUBIC");
                    if (shonlinear) mat.EnableKeyword("BAKERY_SHNONLINEAR");
                    if (volRotY) mat.EnableKeyword("BAKERY_VOLROTATIONY");
                    if (volRot) mat.EnableKeyword("BAKERY_VOLROTATION");
                    if (bicubic) mat.EnableKeyword("BAKERY_BICUBIC");
                    mat.SetFloat("_BAKERY_LMSPECMODE_OCCLUDEREFLECTION_MUL", roMult);
                    mat.SetFloat("_BAKERY_LMSPECMODE_OCCLUDEREFLECTION_ADD", roAdd);
                    mat.SetFloat("_BAKERY_LMSPECMODE_OCCLUDEREFLECTION_SMT", roSMT);
                }
            ...
            else if (!mat.IsKeywordEnabled("USEBAKERY"))
            {
                ...
                if (mat.IsKeywordEnabled("BAKERY_VOLROTATIONY")) mat.DisableKeyword("BAKERY_VOLROTATIONY");
            }
        }
        ...
        public void OnLitGUI(MaterialEditor materialEditor, MaterialProperty[] props)
        {
            ...
            bool lodCrossfade = mat.IsKeywordEnabled("_LOD_FADE_CROSSFADE");
            bool lodCrossfadeNew = EditorGUILayout.Toggle("LOD Crossfade", lodCrossfade);
            if (lodCrossfadeNew != lodCrossfade)
            {
                mat.DisableKeyword("_LOD_FADE_CROSSFADE");
                if (lodCrossfadeNew)
                {
                    mat.EnableKeyword("_LOD_FADE_CROSSFADE");
                }
            }
        ...
        {
            if (FindProperty("_GradientX", props) == null)
                return;
