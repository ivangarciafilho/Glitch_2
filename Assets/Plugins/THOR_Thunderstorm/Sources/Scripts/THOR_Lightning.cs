...
namespace THOR
{
    public class THOR_Lightning : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            if (rt.usesHDRP) lightningBoltMaterial.shader = Shader.Find("THOR_Thunderstorm/THOR_LightningBolt_HDRP");
            else lightningBoltMaterial.shader = Shader.Find("THOR_Thunderstorm/THOR_LightningBolt");
            // Clone Materials
        ...
        IEnumerator LerpEvolution()
        {
            ...
            while (Time.time - tStamp < duration)
            {
                ...
                if (rt.useLight)
                {
                    ...
                    spotLight.intensity = rt.lightIntensityCurve.Evaluate(lerpValue) * lightdist * rt.lightIntensityMulti * (rt.usesHDRP ? 1000000 : 1);
                }
                if (flicker)
                {
