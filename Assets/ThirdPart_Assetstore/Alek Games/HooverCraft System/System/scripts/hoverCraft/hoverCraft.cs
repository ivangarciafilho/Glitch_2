...
namespace AlekGames.HoverCraftSystem.Systems.Main
{
    ...
    public class hoverCraft : MonoBehaviour
    {
                ...
                switch (turnType)
                {
                    case turnT.independent:
                        ...
                        if (inputInvertType != invertType.none)
                        {
                            ...
                            else if (inputInvertType == invertType.velocity && transform.InverseTransformDirection(rb.linearVelocity).z < 0.1f) correctedInput.x *= -1;
                        }

                        break;
                    case turnT.dependent:
                        Vector2 vel = Vector2.ClampMagnitude(new Vector2(rb.linearVelocity.x, rb.linearVelocity.z) / maxSpeed, 1);
                        ...
                        if (inputInvertType != invertType.none)
                        {
                            ...
                            else if(inputInvertType == invertType.velocity && transform.InverseTransformDirection(rb.linearVelocity).z < 0.1f) mult *= -1;
                        }
        ...
        {
            if (rb.linearVelocity.magnitude <= maxSpeed && thrust.z != 0)
            {
        ...
        private void addCounterForce()
        {
            ...
            Vector3 countedVel = rb.linearVelocity;
            countedVel.y = 0;
