...
namespace VolFx
{
    ...
    public class PixelationPass : VolFx.Pass
    {
        ...
        private static int          s_RefResolution = 720;
        ...
        [Tooltip("Default crisp value")]
        public bool           _crispDefault;
        ...
        public override bool Validate(Material mat)
        {
            ...
            _validateMat(mat, usePalette, settings.m_Crisp.overrideState ? settings.m_Crisp.value : _crispDefault);

            var scale  = _scaleLerp.Evaluate(settings.m_Scale.value);

            var height = (float)Mathf.CeilToInt(scale * (float)Screen.height);
            if (_absolute)
                height *= s_RefResolution / (float)Screen.height;
            ...
            var pixels = new Vector4(height * aspect, height, (height / Screen.height) < _gridReveal ? gridscale * .5f : 1f , settings.m_Impact.value);

            mat.SetVector(s_Pixels, pixels);
            mat.SetFloat(s_Roundness, Mathf.Lerp(1.4142f, 1f, roundness));
            mat.SetColor(s_Color, settings.m_Color.value);
            return true;
        }
        ...
        private void _validateMat(Material mat, bool palette, bool crisp)
        {
            ...
            if (_crispLast != crisp)
            {
                ...
                if (crisp) mat.EnableKeyword("_CRISP");
