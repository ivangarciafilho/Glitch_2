...
namespace MDPackage_Editor
{
    ...
    public class MDM_SculptingLite_Editor : MD_ModifierBase_Editor
    {
        ...
        private bool editorInEditMode = false;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (md.sculptingUseInput && !md.ThreadUseMultithreading)
                MDE_DrawProperty("sculptingUpdateColliderAfterRelease", "Update Collider After Release", "If disabled, the collider will be updated every frame if the input is triggered");
            else if (md.ThreadUseMultithreading && !md.sculptingUpdateColliderAfterRelease)
                md.sculptingUpdateColliderAfterRelease = true;

            MDE_DrawProperty("sculptingRecalculateNormalsOnRelease", "Recalculate Normals On Release", "If enabled, the normals recalculation will be processed after the sculpting-button release");
            MDE_s(5);
            MDE_v();
            ...
            MDE_v();
            MDE_DrawProperty("sculptingUseBrushProjection", "Use Brush Projection", "If enabled, the sculpting will use a certain brush gameObject - this helps you to see where the sculpting pointer/brush actually is according to raycast");
            if(md.sculptingUseBrushProjection)
            ...
            if (!md.sculptingMobileSupport)
            {
                ...
                if (md.sculptingUseInput)
                {
                    ...
                    {
                        MDE_hb($"Please hook up your own input to\n\n'{nameof(md.InputHook_CursorScreenPosition)}' for mouse screen position" +
                          $"\n\nproperty or use the example input wrapper from the example content pack");
                        MDE_v();
                        ...
                        if (md.sculptingUseRaiseFunct)
                            MDE_hb($"Please hook up your own input to\n\n'{nameof(md.InputHook_Raise)}'" +
                                $"\n\nproperty or use the example input wrapper from the example content pack");
                        if (md.sculptingUseLowerFunct)
                            MDE_hb($"Please hook up your own input to\n\n'{nameof(md.InputHook_Lower)}'" +
                          $"\n\nproperty or use the example input wrapper from the example content pack");
                        if (md.sculptingUseRevertFunct)
                            MDE_hb($"Please hook up your own input to\n\n'{nameof(md.InputHook_Revert)}'" +
                           $"\n\nproperty or use the example input wrapper from the example content pack");
                        if (md.sculptingUseNoiseFunct)
                            MDE_hb($"Please hook up your own input to\n\n'{nameof(md.InputHook_Noise)}'" +
                          $"\n\nproperty or use the example input wrapper from the example content pack");
                        if (md.sculptingUseSmoothFunct)
                            MDE_hb($"Please hook up your own input to\n\n'{nameof(md.InputHook_Smooth)}'" +
                          $"\n\nproperty or use the example input wrapper from the example content pack");
                        if (md.sculptingUseStylizeFunct)
                            MDE_hb($"Please hook up your own input to\n\n'{nameof(md.InputHook_Stylize)}'" +
                          $"\n\nproperty or use the example input wrapper from the example content pack");
                    ...
                    {
                        MDE_hb($"Please hook up your own input to\n\n'{nameof(md.InputHook_GenericNonPCButton)}'" +
                          $"\n\nproperty to trigger sculpting or use the example input wrapper from the example content pack");
                        MDE_v();
            ...
            else
            {
                MDE_s(3);
                MDE_v();
                MDE_hb($"Please hook up your own input to\n\n'{nameof(md.InputHook_CursorScreenPosition)}' for mouse/touch screen position" +
                          $"\n\nproperty or use the example input wrapper from the example content pack");
                MDE_hb($"Please hook up your own input to\n\n'{nameof(md.InputHook_GenericNonPCButton)}'" +
                          $"\n\nproperty to trigger sculpting or use the example input wrapper from the example content pack");
                MDE_DrawProperty("sculptingCameraCache", "Main Camera Cache", "Assign main camera to make the raycast work properly");
        ...
        private void OnSceneGUI()
        {
            ...
            if (!editorInEditMode)
                return;
            if (md.sculptingBrushProjection == null)
                return;
