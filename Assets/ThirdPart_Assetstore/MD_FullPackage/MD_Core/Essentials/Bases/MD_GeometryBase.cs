...
namespace MDPackage.Geometry
{
    ...
    /// Base geometry class for all the geometry-related objects. Implement this base class to any script that will work with Unity-meshes and frequent updates will be necessary.
    ...
    public abstract class MD_GeometryBase : MD_MeshBase
    {
        ...
        {
            public bool Initialized => vertices != null;
        ...
        // Serialized Private/Protected Fields

        [SerializeField] protected MDVertices geometryVertices;
        [SerializeField] protected MDTriangles geometryTriangles;
        [SerializeField] protected MDUvs geometryUVs;

        [SerializeField] private string geometryName;
        ...
        public string GeometryName => geometryName;

        public MDVertices GeometryVertices => geometryVertices;
        public MDTriangles GeometryTriangles => geometryTriangles;
        public MDUvs GeometryUVs => geometryUVs;
        ...
        {
            if (!sender.TryGetComponent(out Renderer rend))
                return null;

            Shader shad = MD_Utilities.Specific.GetProperPipelineDefaultShader();
        ...
        /// </summary>
        /// <param name="affectUpdateEveryFrameField">Affect 'Update every frame' field? This field may get disabled if vertex count is over the limit</param>
        ...
        {
            if (!MD_Utilities.Specific.RestrictFromOtherTypes(this.gameObject, this.GetType(), new System.Type[3] { typeof(MD_ModifierBase), typeof(MD_GeometryBase), typeof(MD_MeshProEditor) }))
            {
#if UNITY_EDITOR
                if (!Application.isPlaying && MD_Preferences.SelectPreferencesAsset().PopupEditorWindowIfAnyNotification)
        ...
        /// <summary>
        /// Update current mesh with working geometry data (This will not recalculate normals or bounds. Call MDMeshBase_RecalculateMesh instead)
        /// </summary>
        ...
        {
            geometryResolution = Mathf.Max(1, (int)res);
            if(!updateEveryFrame)
                MDMeshBase_ProcessCompleteMeshUpdate();
        }
    ...
    public abstract class MD_GeometryBase_Editor : MD_MeshBase_Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (mGeoBase.IsInitialized)
            {
                ...
                if (!mGeoBase.updateEveryFrame && MDE_b("Flip", 40))
                {
                ...
                MDE_l("Triangles: " + (mGeoBase.GeometryTriangles.triangles.Length / 3).ToString());
