...
namespace MDPackage.Modifiers
{
    ...
    /// Base modifier class for all the modifier objects. Implement this base class to any script that should behave like a 'mesh-modifier'.
    ...
    public abstract class MD_ModifierBase : MD_MeshBase
    {
        ...
        /// <summary>
        /// Does the current mesh-modifier support multithreading feature? (Read only)
        /// </summary>
        ...
        /// <summary>
        /// Create new modifier of a specific type
        /// </summary>
        ...
        public static T CreateModifier<T>(GameObject entry, MeshReferenceType meshReferenceType) where T : MD_ModifierBase
        {
            ...
            m.MDModifier_InitializeBase(meshReferenceType, true);
            return m;
        }

        #region MD_MeshBase implementation
        ...
        protected virtual void MDModifier_InitializeBase(MeshReferenceType meshReferenceType = MeshReferenceType.GetFromPreferences, bool forceInitialization = false, bool affectUpdateEveryFrameField = true)
        {
            ...
            {
                MD_Debug.Debug(this, $"The modifier is already initialized. If you would like to initialize this modifier again, call this method with '{nameof(forceInitialization)}' true parameter");
                return;
            }
            ...
            if(!MD_Utilities.Specific.PrepareMeshDeformationModifier(this, MbMeshFilter, new Type[3] { typeof(MD_ModifierBase), typeof(MD_GeometryBase), typeof(MD_MeshProEditor) }, true, meshReferenceType))
            {
            ...
            if (this is IMDThreadingSupport imdts)
            {
                ...
                imdts.ThreadUseMultithreading = MD_Preferences.SelectPreferencesAsset().VertexLimit < MbMeshFilter.sharedMesh.vertexCount;
            }
            else MbMultithreadedModifier = false;

            MbIsInitialized = true;
            OnModifierInitialized?.Invoke();
        }
        ...
        {
            if (MbIsInitialized && updateEveryFrame)
                MDModifier_ProcessModifier();
        }
        ...
        public void MDModifier_SubdivideMesh(int level = 2)
        {
            ...
            if (MD_Utilities.Specific.CheckVertexCountLimit(MbWorkingMeshData.vertices.Length * level, gameObject))
            {
                OnMeshSubdivided?.Invoke();

                MD_Utilities.MeshSubdivision.Subdivide(MbMeshFilter.sharedMesh, level);
                MDModifier_InitializeMeshData(false, MbBackupMeshData.MbDataInitialized());
                MDMeshBase_RecalculateMesh();
            }
        }
        ...
        public void MDModifier_SmoothMesh(float intensity = 0.5f)
        {
            ...
            smoothVerts.vertices = MD_Utilities.VertexSmoothingHCFilter.HCFilter(MbWorkingMeshData.vertices, MbWorkingMeshData.triangles, 0.0f, intensity);
        ...
        public void MDModifier_CreateNewMeshReference(string meshName = "")
        {
            ...
            MbMeshFilter.sharedMesh = MD_Utilities.Specific.CreateNewMeshReference(MbMeshFilter.sharedMesh);
        ...
        private bool MDModifierThreading_CheckInit(bool checkForRunningThread = false)
        {
            ...
            if(checkForRunningThread)
            {
                ...
                {
                    MD_Debug.Debug(this, "The modifier thread is already alive & running", MD_Debug.DebugType.Info);
                    return false;
                }
            }
            return true;
...
namespace MDPackage_Editor
{
    ...
    public abstract class MD_ModifierBase_Editor : MD_MeshBase_Editor
    {
        ...
        protected MD_Preferences preferences;
        ...
        {
            preferences = MD_Preferences.SelectPreferencesAsset();
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if(mModifierBase.MbMultithreadedModifier)
            {
                ...
                MDE_v();
                if (mModifierBase.MbWorkingMeshData.vertices.Length > preferences.VertexLimit)
                    MDE_hb("The multithreading is recommended if the mesh has more than " + preferences.VertexLimit.ToString() + " vertices", MessageType.Warning);
