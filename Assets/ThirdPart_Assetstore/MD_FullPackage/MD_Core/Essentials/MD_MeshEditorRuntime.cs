...
namespace MDPackage
{
    ...
    /// </summary>
    [AddComponentMenu(MD_Debug.ORGANISATION + MD_Debug.PACKAG_PATH_NAME + "Mesh Editor Runtime")]
    public sealed class MD_MeshEditorRuntime : MonoBehaviour, IDMGenericInputTemplate
    {
        ...
        private bool nonAxis_InputDown;
        ...
        private List<PotentialPoints> nonAxis_potentialPoints;
        ...
        private bool axis_AxisObjectCreatedAutomatically = false;
        [SerializeField] private MD_MeshProEditor axis_targetMeshProEditor;
        ...
        private readonly MD_BackendGenericButton selectionButtonBacked = new MD_BackendGenericButton();
        ...
        private void Start () 
        {
            ...
            if (isAxisEditor)
            {
                ...
                nonAxis_potentialPoints = new List<PotentialPoints>();
        ...
        private void Update () 
        {
            ...
            {
                selectionButtonBacked.SyncButton(InputHook_GenericButtonDown);

                InternalProcess_AxisEditor();

                selectionButtonBacked.ResetButton(false);
            }
        }

        private void OnDestroy()
        {
            if (axis_AxisObjectCreatedAutomatically && axis_axisObject != null)
                Destroy(axis_axisObject);
            if (selectionHelper != null)
                Destroy(selectionHelper.gameObject);
            if (ppAXIS_GroupSelector != null)
                Destroy(ppAXIS_GroupSelector);

            ppAXIS_TotalPoints.Clear();
            ppAXIS_SelectedPoints.Clear();
            ppAXIS_UndoStoredObjects.Clear();
            nonAxis_potentialPoints = null;
        }
        ...
        {
            if (!nonAxis_cursorIsOrigin && nonAxis_handleCursor && nonAxis_lockAndHideCursor && !nonAxis_isMobileFocused)
            {
            ...
            if (nonAxis_InputDown && nonAxis_potentialPoints.Count > 0)
            {
                ...
                else
                {
                    ...
                    Vector3 futurePos = mainCameraCache.ScreenToWorldPoint(new Vector3(
                        nonAxis_lockAxisX ? mainCameraCache.WorldToScreenPoint(origPos).x : InputHook_CursorScreenPosition.x,
                        nonAxis_lockAxisY ? mainCameraCache.WorldToScreenPoint(origPos).y : InputHook_CursorScreenPosition.y, 
            ...
            if (!nonAxis_switchAppearance && !InputHook_GenericButtonDown) return;
            ...
            {
                if (nonAxis_cursorIsOrigin)
                    ray = mainCameraCache.ScreenPointToRay(InputHook_CursorScreenPosition);
                else
            ...
            if (InputHook_GenericButtonDown)
            {
                ...
                selectionHelper.position = mainCameraCache.ScreenToWorldPoint(new Vector3(InputHook_CursorScreenPosition.x, InputHook_CursorScreenPosition.y, (mainCameraCache.transform.position - centerPoint).magnitude));
                foreach (PotentialPoints tr in nonAxis_potentialPoints)
                {
                    if(nonAxis_vertexControlMode == VertexControlMode.GrabDropVertex)
        ...
        {
            //---BEFORE SELECTION
            if (selectionButtonBacked.IsDown)
            {
                Ray ray = mainCameraCache.ScreenPointToRay(InputHook_CursorScreenPosition);
                if (Physics.Raycast(ray, out RaycastHit hit))
                {
                    ...
                    if(!hitt)
                    {
                        if(hit.collider.transform.parent != null)
                        {
                            ...
                            {
                                if (InputHook_AxisEditorAddPointsToSelection && hit.collider.transform.parent == axis_targetMeshProEditor.GeneratedPointsRootContainer)
                                {
                                ...
                                else if (InputHook_AxisEditorRemovePointsFromSelection && hit.collider.transform.parent == ppAXIS_GroupSelector.transform)
                                {
                                    hit.collider.gameObject.GetComponentInChildren<Renderer>().material.color = axis_pointColorStorage;
                                    hit.collider.gameObject.transform.parent = axis_targetMeshProEditor.GeneratedPointsRootContainer;
                            ...
                            else if (ppAXIS_SelectedPoints.Count == 0 && hit.collider.transform.parent == axis_targetMeshProEditor.GeneratedPointsRootContainer)
                            {
                                ppAXIS_SelectedPoints.Add(hit.collider.gameObject.transform);
                ...
                ppAXIS_CursorPosOrigin = InputHook_CursorScreenPosition;
                if (!InternalAxis_CheckSideFunctions())
                {
                    if (ppAXIS_SelectedPoints.Count > 0)
                    {
                        ...
                        foreach (Transform t in ppAXIS_SelectedPoints)
                        {
                            ...
                            t.transform.parent = axis_targetMeshProEditor.GeneratedPointsRootContainer;
            ...
            if(Axis_IsMoving)
            {
                if (axis_movePointsToAxis == AxisMoveAxisTo.X)
                {
                    ...
                    if (mainCameraCache.transform.position.z > axis_axisObject.transform.position.z)
                        PosFix *= -1;
                    ppAXIS_GroupSelector.transform.position += (ppAXIS_GroupSelector.transform.right * (InputHook_AxisEditorMousePositionDelta.x * PosFix) * axis_manipulationSpeed) * Time.deltaTime;
                }
                if (axis_movePointsToAxis == AxisMoveAxisTo.Y)
                    ppAXIS_GroupSelector.transform.position += (ppAXIS_GroupSelector.transform.up * InputHook_AxisEditorMousePositionDelta.y * axis_manipulationSpeed) * Time.deltaTime;
                if (axis_movePointsToAxis == AxisMoveAxisTo.Z)
                {
                    float PosFix = 1;
                    if (mainCameraCache.transform.position.x < axis_axisObject.transform.position.x)
                        PosFix *= -1;
                    ppAXIS_GroupSelector.transform.position += (ppAXIS_GroupSelector.transform.forward * (InputHook_AxisEditorMousePositionDelta.x * PosFix) * axis_manipulationSpeed) * Time.deltaTime;
                }
            ...
            if (selectionButtonBacked.IsUp)
            {
                ...
                foreach (Transform t in ppAXIS_TotalPoints)
                {
                    if (t == null)
                        continue;
                    if (AxisEditor_Utilities.IsInsideSelection(mainCameraCache, ppAXIS_CursorPosOrigin, InputHook_CursorScreenPosition, t.gameObject))
                    {
                        if (!InputHook_AxisEditorRemovePointsFromSelection)
                        {
                            if (c == 0)
        ...
        private bool InternalAxis_CheckSideFunctions() => InputHook_AxisEditorAddPointsToSelection || InputHook_AxisEditorRemovePointsFromSelection;
        ...
        private void InternalAxis_RefreshBounds(Transform center = null)
        {
            ...
            {
                if (!center)
                    ppAXIS_GroupSelector.transform.rotation = axis_targetMeshProEditor.GeneratedPointsRootContainer.rotation;
                else
        ...
        private void OnGUI()
        {
            ...
            if (Axis_IsSelecting)
            {
                var rect = AxisEditor_Utilities.GetScreenRect(ppAXIS_CursorPosOrigin, InputHook_CursorScreenPosition);
        ...
        public void AXIS_SwitchTarget(MD_MeshProEditor Target)
        {
            ...
            if (!axis_targetMeshProEditor.GeneratedPointsRootContainer)
            {
            ...
            foreach (Transform t in axis_targetMeshProEditor.GeneratedPointsRootContainer)
        ...
        public void AXIS_CreateAxisHandleAutomatically()
        {
            ...
            Material mat1 = new Material(Utilities.MD_Utilities.Specific.GetProperPipelineDefaultShader());
            mat1.color = Color.red;
            X_Axis.GetComponent<Renderer>().material = mat1;
            Material mat2 = new Material(Utilities.MD_Utilities.Specific.GetProperPipelineDefaultShader());
            mat2.color = Color.green;
            Y_Axis.GetComponent<Renderer>().material = mat2;
            Material mat3 = new Material(Utilities.MD_Utilities.Specific.GetProperPipelineDefaultShader());
            mat3.color = Color.blue;
            Z_Axis.GetComponent<Renderer>().material = mat3;

            axis_axisObject = AxisRoot;

            axis_AxisObjectCreatedAutomatically = true;
        }
        ...
        private static class AxisEditor_Utilities
        {
            ...
            //---Checking Selection
            public static bool IsInsideSelection(Camera camSender, Vector3 originCursor, Vector3 targetCursor, GameObject ObjectInsideSelection)
            {
                Camera camera = camSender;
                Bounds viewportBounds = GetViewportBounds(camera, originCursor, targetCursor);
    ...
    public sealed class MD_MeshEditorRuntime_Editor : MD_EditorUtilities
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (m.isAxisEditor)
            {
                ...
                MDE_l("Selection Essentials", true);
                MDE_v();
                ...
                MDE_l("Controls", true);
                MDE_v();
                MDE_hb($"Please hook up your own input to\n\n'{nameof(m.InputHook_GenericButtonDown)}' for vertex selection, " +
                    $"\n'{nameof(m.InputHook_AxisEditorAddPointsToSelection)}' for vertex add to the current selection, " +
                    $"\n'{nameof(m.InputHook_AxisEditorRemovePointsFromSelection)}' for vertex removal from the current selection, " +
                    $"\n'{nameof(m.InputHook_AxisEditorMousePositionDelta)}' for vertex movement by mouse delta position and " +
                    $"\n'{nameof(m.InputHook_CursorScreenPosition)}' for cursor/touch finger screen position " +
                    $"\n\nproperties or use the example input wrapper from the example content pack");
                MDE_ve();
                MDE_s(5);

                MDE_l("Selection Visuals", true);
                MDE_v(); MDE_DrawProperty("axis_inLocalSpace", "Local Space", "Axis object orientation");
            ...
            MDE_v();
            MDE_hb($"Please hook up your own input to\n\n" +
                $"'{nameof(m.InputHook_GenericButtonDown)}' for vertex selection and" +
                $"\n'{nameof(m.InputHook_CursorScreenPosition)}' for cursor/touch finger screen position\n\nproperties or use the example input wrapper from the example content pack") ;
            MDE_ve();
            MDE_s(5);
            ...
            if (!m.nonAxis_isMobileFocused)
            {
                ...
                {
                    MDE_DrawProperty("nonAxis_handleCursor", "Handle Screen Cursor", default, default, true);
                    if(m.nonAxis_handleCursor)
