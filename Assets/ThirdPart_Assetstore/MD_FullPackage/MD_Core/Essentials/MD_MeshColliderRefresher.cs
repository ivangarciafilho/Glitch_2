...
namespace MDPackage
{
    ...
    /// </summary>
    [AddComponentMenu(MD_Debug.ORGANISATION + MD_Debug.PACKAG_PATH_NAME + "Mesh Collider Refresher")]
    public sealed class MD_MeshColliderRefresher : MonoBehaviour
    {
        ...
        [field:SerializeField] public MeshCollider MeshColliderCache { get; private set; }
        ...
        private void Awake()
        {
            ...
            if (!MeshColliderCache) MeshColliderCache = GetComponent<MeshCollider>();
        ...
        public void MeshCollider_UpdateMeshCollider()
        {
            ...
            if (!MeshColliderCache) MeshColliderCache = GetComponent<MeshCollider>(); 
            if (!MeshColliderCache) MeshColliderCache = gameObject.AddComponent<MeshCollider>();

            MeshColliderCache.sharedMesh = selfRenderCache.sharedMesh;
            MeshColliderCache.convex = convexMeshCollider;
            MeshColliderCache.cookingOptions = cookingOptions;
            ...
            newMeshCol.vertices = MeshColliderCache.sharedMesh.vertices;
            newMeshCol.triangles = MeshColliderCache.sharedMesh.triangles;
            newMeshCol.normals = MeshColliderCache.sharedMesh.normals;
            Vector3[] verts = newMeshCol.vertices;
            for (int i = 0; i < verts.Length; i++)
                verts[i] += colliderOffset;
            newMeshCol.vertices = verts;
            MeshColliderCache.sharedMesh = newMeshCol;
        }
    }
}
