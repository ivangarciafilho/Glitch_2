...
namespace MDPackage_Editor
{
    ...
    public abstract class MD_EditorUtilities : Editor
    {
        ...
        {
            if (sender.GetComponent<MeshCollider>())
                return;

            ColorUtility.TryParseHtmlString("#a6e0b5", out Color c);
            Color oc = GUI.color;
            GUI.color = c;
            if (MDE_b("Add Mesh Collider"))
                sender.AddComponent<MeshCollider>();
            GUI.color = oc;
        }
        protected void MDE_AddMeshColliderRefresher(GameObject sender)
        {
            if (sender.GetComponent<MD_MeshColliderRefresher>())
                return;
            ...
            if (MDE_b("Add Mesh Collider Refresher"))
                sender.AddComponent<MD_MeshColliderRefresher>();
            GUI.color = oc;
        }
