...
namespace MDPackage_Editor
{
    ...
    public sealed class MD_VertexTool : MD_EditorWindowUtilities
    {
        ...
        private void OnGUI()
        {
            ...
            if (Selection.activeGameObject != null && Selection.activeGameObject.GetComponent<MD_MeshProEditor>())
                if (GUILayout.Button("Group Enabled"))
                {
                    MD_MeshProEditor p = Selection.activeGameObject.GetComponent<MD_MeshProEditor>();
                    if (p.GeneratedMeshPoints.Count == 0 || GroupObject == null)
                        return;
                    foreach (Transform gm in p.GeneratedMeshPoints)
                    {
    ...
    public static class MD_VertexToolUtility
    {
...
#endif
            attachToGameObject.GetComponent<MD_MeshProEditor>().MPE_CombineMeshImmediately();
        }
        ...
        public static void V_TOOL_Element_Clone(GameObject targetGameObject, int count, Vector3 rotationOffset, Vector3 positionOffset)
        {
            ...
            Vector3 offset = targetGameObject.transform.position 
                + new Vector3(mf.sharedMesh.bounds.max.x * positionOffset.x, mf.sharedMesh.bounds.max.y * positionOffset.y, mf.sharedMesh.bounds.max.z * positionOffset.z);
            ...
            mpe.MPE_CombineMeshImmediately();
        }
        ...
        public static void V_TOOL_Vertex_Relax(MD_MeshProEditor mpeSender)
        {
            if (mpeSender == null)
                return;
            if (mpeSender.GeneratedMeshPoints == null)
                return;
            if (mpeSender.GeneratedMeshPoints.Count > 0)
            {
                foreach (Transform points in mpeSender.GeneratedMeshPoints)
                {
