...
namespace MDPackage.Modifiers
{
    ...
    [AddComponentMenu(MD_Debug.ORGANISATION + MD_Debug.PACKAG_PATH_NAME + "Modifiers/Interactive Surface")]
    public sealed class MDM_InteractiveSurface : MD_ModifierBase, MD_ModifierBase.IMDThreadingSupport
    {
        ...
        /// <summary>
        /// Use 'InteractiveSurface_ModifyMesh' method for more customized setting
        /// </summary>
...
namespace MDPackage_Editor
{
    ...
    public sealed class MDM_InteractiveLandscape_Editor : MD_ModifierBase_Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            MDE_v();
            MDE_DrawProperty("direction", "Direction Of Deformation", "Main direction of the vertices after interaction", identOffset: true);
            MDE_v();
            MDE_DrawProperty("exponentialDeformation", "Exponential Deform", "If enabled, the mesh will be deformed expontentially", identOffset: true);
            ...
            if (mb.rigidbodiesAllowed)
            {
                ...
                if (!mb.detectRadiusSize)
                    MDE_DrawProperty("radius", "Interaction Radius", "Radius of vertices to be interacted with. Please keep in mind that if the input for mesh modify is RayEvent with Spherical Raycast, this field is no more required as the radius is received from the RayEvent - Non-Pointed Ray Radius", identOffset: true);
                MDE_ve();
            }
