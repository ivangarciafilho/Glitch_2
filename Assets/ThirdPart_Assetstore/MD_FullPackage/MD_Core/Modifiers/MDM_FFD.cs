...
namespace MDPackage.Modifiers
{
    ...
    [AddComponentMenu(MD_Debug.ORGANISATION + MD_Debug.PACKAG_PATH_NAME + "Modifiers/FFD")]
    public sealed class MDM_FFD : MD_ModifierBase
    {
        ...
        public override void MDModifier_ProcessModifier()
        {
            ...
            if (initialNodePositions.Count == 0) return;
            if (generatedNodes.Count != initialNodePositions.Count) return;
            if (!MbWorkingMeshData.MbDataInitialized()) return;
        ...
        public void FFD_RefreshFFDGrid()
        {
            ...
            int ffd = Mathf.Max(1, ffdCustomCount - 1);
        ...
        public void FFD_ClearFFDGrid()
        {
            for(int i = generatedNodes.Count - 1; i >= 0; i--)
            {
                ...
                if (Application.isPlaying)
                    ...
                    DestroyImmediate(generatedNodes[i].gameObject);
            }
        ...
        public IReadOnlyList<Transform> FFD_GetNodes()
            => generatedNodes;
